...
def false_positive_example(algo="jactivemodules_greedy",dataset="TNFa_2", is_corrected=False, axs=None):
    ...
    for cur_bg in np.arange(n_of_bg):
        ...
        sns.distplot(values,norm_hist=False, kde=False, label="Permuted dataset", ax=axs[0], bins=50)
    ...
    axs[0].set_title("enrichment signal histogram\ndataset: {} algorithm: {}".format(dataset, algo), fontsize=fontsize)
...
if __name__ == "__main__":
    ...
    for is_corrected in [False, True]:
        ...
        i=0
        fig,axs=plt.subplots(len(datasets)*len(algos), 2, figsize=(25, 8*len(datasets)*len(algos)))
        ...
        fig.text(0.01,0.75, "C:", weight='bold',fontsize=fontsize)
        fig.text(0.55, 0.75, "D:", weight='bold',fontsize=fontsize)
