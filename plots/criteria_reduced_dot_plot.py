...
def plot_scatter_with_size(df_size, df_ratio, ax=None, title=""):
...
def dot_grid(df_measurements, y_label, filter_zeros=False, ax=None, algos=None, title=""):
    ...
    for i, cur_row in df_measurements.iterrows():
        for cur_entry in cur_row:
            df_new=df_new.append({"algo": constants.ALGOS_ACRONYM[i], y_label : cur_entry}, ignore_index=True)

    df_new=df_new.dropna(axis=0)
    my_order = constants.ALGOS_ACRONYM.values() # df_new.groupby(by=["algo"])[y_label].mean().sort_values().index
    ax = sns.stripplot(x='algo', y=y_label, data=df_new, jitter=True, size=15, ax=ax, order=my_order, palette={a: "#AAAAAA" for a in my_order}) # {a: colorlist[algo_acrs.index(a)] for a in my_order}
...
def main():
    ...
    dot_grid(df_measurements_counts, "# GO terms", ax=axs_4[1][0], title="Term counts, GE")
    ...
    plot_scatter_with_size(df_measurements_ratio, df_measurements_richness, ax=axs_5[0], title="EHR-Richness, GE")
    ...
    main_path = "/home/hag007/Desktop/aggregate_gwas_report/venn"
    df_measurements_counts = pd.read_csv(os.path.join(main_path, "count_matrix.tsv"), sep='\t', index_col=0) # .loc[np.sort(constants.ALGOS_ACRONYM.keys()),:] # ratio_matrix.tsv
    dot_grid(df_measurements_counts, "# GO terms", ax=axs_4[1][1], title="Term counts, GWAS")
    ...
    plot_scatter_with_size(df_measurements_ratio, df_measurements_richness, ax=axs_5[1], title="EHR-Richness, GWAS")
