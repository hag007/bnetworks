...
def plot_richness_homogeneity(df_richness, df_homogeneity, grid_type, ax=None, title=""):
    df_richness=df_richness.loc[np.sort(df_richness.index), np.sort(df_richness.columns)].loc[constants.ALGOS]
    df_homogeneity=df_homogeneity.loc[np.sort(df_homogeneity.index), np.sort(df_homogeneity.columns)].loc[constants.ALGOS]
    ...
    ax.legend(handles=constants.PATCHES, loc=(0,1.1), ncol=2, prop={'size': 20}, facecolor='#ffffff')
...
def plot_richness_homogeneity_agg(df_richness, df_homogeneity, grid_type, ax=None, title=""):
    df_richness=df_richness.loc[np.sort(df_richness.index), np.sort(df_richness.columns)].loc[constants.ALGOS]
    df_homogeneity=df_homogeneity.loc[np.sort(df_homogeneity.index), np.sort(df_homogeneity.columns)].loc[constants.ALGOS]
...
def main():
    ...
    for cutoff in cutoffs:
        ...
        df_richness=pd.read_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "solution_richness_matrix_{}.tsv".format(suffix)), sep='\t', index_col=0).loc[np.sort(constants.ALGOS_ACRONYM.keys()),:]
        df_homogeneity = pd.read_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "homogeneity_avg_matrix_{}.tsv".format(suffix)), sep='\t', index_col=0).loc[np.sort(constants.ALGOS_ACRONYM.keys()),:]
        plot_richness_homogeneity(df_richness, df_homogeneity, "size", ax=axs_1[0], title="GE")
        plot_richness_homogeneity_agg(df_richness, df_homogeneity, "size", ax=axs_2[0], title="GE")


        suffix="PASCAL_SUM_{}".format(cutoff)
        df_richness=pd.read_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "solution_richness_matrix_{}.tsv".format(suffix)), sep='\t', index_col=0).loc[np.sort(constants.ALGOS_ACRONYM.keys()),:]
        df_homogeneity = pd.read_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "homogeneity_avg_matrix_{}.tsv".format(suffix)), sep='\t', index_col=0).loc[np.sort(constants.ALGOS_ACRONYM.keys()),:]
        plot_richness_homogeneity_agg(df_richness, df_homogeneity, "size", ax=axs_2[1], title="GWAS")
