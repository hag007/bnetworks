    ...
    for hg_pval in hg_pvals:
        ...
        if len(pval) < n_permutation:
            ...
            if row['GO name'] == "lymphocyte migration":
                print "pos: {}, hg_pval: {}, max dist: {}".format(pos, hg_pval, np.max(np.append(pval, [0])))
                x = 1
...
def main(dataset="SOC", algo="jactivemodules_sa", n_permutations=300,
    ...
    if prefix == "GE":
        dataset_data = pd.read_csv(
            os.path.join(constants.DATASETS_DIR, "{}_{}".format(prefix, dataset), "data", "ge.tsv"), sep='\t',
            index_col=0)
        classes_data = np.array(file(
            os.path.join(constants.DATASETS_DIR, "{}_{}".format(prefix, dataset), "data", "classes.tsv")).readlines()[
    ...
    if df_filtered_in.shape[0] != 0:
        ...
        df_filtered_in["emp_pval_max"] = df_filtered_in["emp_pval"].apply(
            lambda a: a if type(a) != str else np.nanmin(np.array([1]+a[1:-1].split(", "), dtype=np.float32)))

        if not dataset_data is None:
            df_filtered_in["mean_difference"] = df_filtered_in.apply(
                lambda x: mean_difference(x, dataset_data, classes_data),
                axis=1)
    ...
    df_all.loc[df_all["hg_pval"].values > 0, :][
        ["GO name", "hg_pval", "hg_pval_max", "emp_pval", "emp_pval_max", "hg_rank", "emp_rank", "n_genes", "depth",
