...
def main(dataset="SOC", algo="jactivemodules_sa", csv_file_name=os.path.join(constants.OUTPUT_GLOBAL_DIR, "emp_fdr","{dataset}_MAX/emp_diff_{dataset}_{algo}_md.tsv" )):
    ...
    # df["emp_pval"]=df["emp_pval"].apply(lambda x: (round(x*float(301),10)-1)/300.0)
    ...
    for index, cur in df_filtered_in.iterrows():
        ...
        try:
            df_filtered_in.loc[index,"mean_difference"] = \
                dataset_data.loc[entrez2ensembl_convertor(get_all_genes_for_term(vertices, index, index,
                                                                                 index == index)), classes_data == 2].dropna().values.mean() - \
                dataset_data.loc[entrez2ensembl_convertor(get_all_genes_for_term(vertices, index, index,
                                                                                 index == index)), classes_data == 1].dropna().values.mean()
        except:
            print "noo gene were found for {}, {}, {}, {} (pval={})".format(dataset,algo, index, cur["GO name"], cur["filtered_pval"])
            df_filtered_in.loc[index, "mean_difference"] =np.NAN
