...
def main(algo_sample=None, dataset_sample=None, n_dist_samples=300, n_total_samples=None, shared_list=None,
         n_start_i=None, limit=10000,
    ...
    n_genes_pvals = output_md.loc[
        np.logical_and.reduce([output_md["n_genes"].values > 5, output_md["n_genes"].values < 500]), "hg_pval"]
    ...
    if n_start_i is None:
        ...
        if n_total_samples > 0 and n_dist_samples >0:
    ...
    file(os.path.join(constants.OUTPUT_GLOBAL_DIR, "consistent_pval.txt"), 'w+').write(
        "\n".join([str(a) for a in max_emp_pvals]))
    print "min vals", 1.0 / max(n_dist_samples,1), np.nanmin(list(max_emp_pvals) + [1])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_total_samples', help="n_total_samples", dest='n_total_samples', default=0)
    parser.add_argument('--n_dist_samples', help="n_dist_samples", dest='n_dist_samples', default=0)
    ...
    for cur_ds in datasets:
        for cur_alg in algos:
            ...
            for BH_TH, n_emp_true, HG_CUTOFF, n_hg_true, go_ids_result, go_names_result, mask_ids, mask_terms in list(
                    shared_list):
            ...
            go_ids_intersection = list(set(go_ids_results[0]).intersection(*go_ids_results)) if len(
                go_ids_results) > 0 else []
            ...
            if type(output_md['emp_pval'].iloc[0]) != str:
                ...
                print "output_md :{}".format(
                    np.min(np.array(output_md['emp_pval'].iloc[0][1:-1].split(", "), dtype=np.float)))
                output_md['emp_pval_max'] = output_md['emp_pval'].apply(
                    lambda a: np.min(np.array(a[1:-1].split(", "), dtype=np.float)) if type(a) == str else "")
            ...
            modules_summary = pd.read_csv(
                os.path.join(constants.OUTPUT_GLOBAL_DIR, "{}_{}".format(prefix, cur_ds), cur_alg,
                             "modules_summary.tsv"), sep='\t')
            full_report = full_report.append(
                {"index": "{}_{}".format(cur_alg, cur_ds), "dataset": cur_ds, "algo": cur_alg,
                 "n_mutual_sig_terms": len(go_names_intersection),
                 "mean_sig_terms": np.mean(ys2) if len(ys2) > 0 else 0, "n_genes": modules_summary["#_genes"].sum(),
                 "n_modules": modules_summary["#_genes"].count(),
                 "modules_size_mean": modules_summary["#_genes"].mean(),
                 "module_size_std": modules_summary["#_genes"].sum()}, ignore_index=True)

            fig, ax = plt.subplots()
            plt.plot(np.arange(len(ys1)), ys1, label='# intersected terms')
            plt.plot(np.arange(len(ys2)), ys2, label='# sig. terms in permutation')
            plt.title(
                "min # terms above threshold ({}):  ({}/{}). ratio: {}/{}".format(terms_limit, len(ys1), n_iteration,
                                                                                  ys1[-1] if len(ys1) > 0 else 0,
                                                                                  min([len(x) for x in
                                                                                       go_names_results]) if len(
                                                                                      go_names_results) > 0 else 0))
            ax.set_xlabel("iteration index")
            ax.set_ylabel("# terms")

            plt.savefig(
                os.path.join(constants.OUTPUT_GLOBAL_DIR, "intersection_function_{}_{}.png".format(cur_ds, cur_alg)))
            ...
            fdr_test_summary.to_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, "emp_fdr",
                                                 "fdr_test_summary_{}_{}.tsv".format(cur_ds, cur_alg)), sep='\t')
    ...
    for cur_ds_i, cur_ds in enumerate(datasets):
        rank_col = np.append(rank_col,
                             full_report["n_mutual_sig_terms"].iloc[cur_ds_i:cur_ds_i + len(algos)].rank(ascending=1))
