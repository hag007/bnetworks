...
def main(datasets, algos, pf=10, base_folder='/home/hag007/Desktop/aggregate_gwas_report/oob', file_format="emp_diff_modules_{}_{}_passed_oob.tsv", sim_method='Resnik'):
    ...
    for cur_ds in datasets:
        ...
        for i_algo, cur_algo in enumerate(algos):
            ...
            cache_file=os.path.join(constants.OUTPUT_GLOBAL_DIR, "similarity_cache_{}_{}_{}.txt".format(sim_method, cur_ds,cur_algo))
            go_terms_reduced, adj = calc_intra_similarity(all_go_terms, pf, emp_results_fdr, cache_file, semsim)

            emp_results_fdr[emp_results_fdr.index.isin(go_terms_reduced)].to_csv(os.path.join(constants.OUTPUT_GLOBAL_DIR, file_format.format(cur_ds,cur_algo)[:-4]+"_reduced.tsv"), sep='\t')
...
def calc_intra_similarity(all_go_terms, pf, enrichment_scores, cache_file, semsim):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pf', dest='pf', default=1)
    parser.add_argument('--base_folder', dest='base_folder', default='/home/hag007/Desktop/aggregate_report/oob')
    ...
    for cur_ds in datasets:
        ...
        main(datasets=datasets, algos=algos, pf=pf, base_folder=base_folder, file_format=file_format, sim_method=sim_method)
